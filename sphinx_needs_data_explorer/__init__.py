...
def add_files(app, config):
    ...
    if "sphinx_needs_data_explorer_config" in app.config:
        if "disable-header-button" in app.config["sphinx_needs_data_explorer_config"]:
            disable_header_button_opt = app.config["sphinx_needs_data_explorer_config"][
    ...
    if not disable_header_button:
        if ("html_theme" in app.config) and (
            ...
            app.config["templates_path"].append(
                path.join(_ROOT_DIR, static_directory, "_templates")
            )
            if not ("html_static_path" in app.config):
                app.config["html_static_path"] = []
            app.config["html_static_path"].append(
                path.join(_ROOT_DIR, static_directory, "_static")
            )

    if sphinx.version_info[:2] >= (5, 0) and not getattr(
        app, "sphinx_needs_data_explorer_installed", False
    ):
        try:
            ...
            raise sphinx_needs_data_explorer_ExtensionError(
                "Something went wrong in my extension"
            )
        ...
        environment = Environment(
            loader=FileSystemLoader(path.join(_ROOT_DIR, static_directory))
        )
        ...
        if "sphinx_needs_data_explorer_config" in app.config:
            ...
            if "valid-linkage" in app.config["sphinx_needs_data_explorer_config"]:
                valid_linkage = app.config["sphinx_needs_data_explorer_config"][
                    "valid-linkage"
                ]
            if (
                "invalid-linkage-color"
                in app.config["sphinx_needs_data_explorer_config"]
            ):
                invalid_linkage_color = app.config["sphinx_needs_data_explorer_config"][
                    "invalid-linkage-color"
                ]
            if "valid-linkage-color" in app.config["sphinx_needs_data_explorer_config"]:
                valid_linkage_color = app.config["sphinx_needs_data_explorer_config"][
                    "valid-linkage-color"
                ]
...
def SphinxNeedsDataExplorer_role(
    ...
    node = nodes.raw(
        ...
        class="custom-reference" title="Follow this link to explore your sphinx-needs data">{text}</a>""",
        format="html",
    )
    return [node], []
